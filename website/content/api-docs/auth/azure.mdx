...
- `subscription_id` `(string: "")` - The subscription ID for the machine that
  ...
  metadata. Required for auth roles configured with `bound_locations`, 
  `bound_subscription_ids`, `bound_resource_groups`, or `bound_scale_sets` constraints.
...
- `vm_name` `(string: "")` - The virtual machine name for the machine that
  generated the MSI token. This information can be obtained through instance
  metadata. `vm_name` or `vmss_name` are required for auth roles configured
  with `bound_locations`, `bound_subscription_ids`, `bound_resource_groups`, or 
  `bound_scale_sets` constraints. If `vmss_name` is provided, this value is ignored.
